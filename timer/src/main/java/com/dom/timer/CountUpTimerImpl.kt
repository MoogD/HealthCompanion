 ...
 * Will call the [CountUpTimer.Listener.onTick] function of the provided listener (if [setListener] was called) every time the [periodInMillis] is reached (default to 1 second).
 ...
 * If [endTimeInMillis] is provided and reached the [CountUpTimer.Listener.onFinish] function of the provided listener (if [setListener] was called) will be called.
 * */
class CountUpTimerImpl(
    ...
    /**
     * Starts the timer by launching a new coroutine job that triggers calling the [CountUpTimer.Listener.onTick] function after a delay of [periodInMillis] in a loop.
    ...
    /**
     * Starts the timer beginning with the last tracked time
     * by launching a new coroutine job that triggers calling the [CountUpTimer.Listener.onTick] function after a delay of [periodInMillis] in a loop.
     * */
    override fun resume() {
        uiScope.coroutineContext.cancelChildren()
        runTimer()
    ...
    /**
     * Stops the timer by canceling the coroutine job and all potentially pending jobs.
     * Also resets the tracked time to [startTimeInMillis]. Returns tracked time.
     * */
